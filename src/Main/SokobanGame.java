...
public class SokobanGame implements Observer {
	...
	private List<AbstractSObject> objs = new ArrayList<AbstractSObject>();
	private int level=0;
	...
	public List<AbstractSObject> readLevel(String numlevel) {
		try {
			Scanner scan = new Scanner(new File("levels//level" + numlevel + ".txt"));	
			int y=0;
		...
		for(AbstractSObject i : list)
			if(!i.isTransponivel()) {
				if(i instanceof ActiveObject) {
					if(obj instanceof Empilhadora && ((Empilhadora) obj).hasMartelo()) 
						return true;
				}
				return false;
			}
